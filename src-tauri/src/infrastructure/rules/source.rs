...
impl RuleSource for JsonRuleSource {
    fn fetch(&self) -> Result<RuleSet> {
        ...
        if !path.exists() {
            return Err(DomainError::IoError(format!("Rule file not found: {}", self.file_path)));
        }
        ...
        let rule_set: RuleSet = serde_json::from_str(&content)
            .map_err(|e| DomainError::SerializationError(format!("Failed to parse JSON rules: {}", e)))?;
...
impl RuleSource for EmbeddedRuleSource {
    fn fetch(&self) -> Result<RuleSet> {
        let rule_set: RuleSet = serde_json::from_str(self.raw_json)
            .map_err(|e| DomainError::SerializationError(format!("Failed to parse embedded JSON rules: {}", e)))?;
