...
impl DatabaseManager {
    pub fn new(custom_dir: Option<PathBuf>) -> Result<Self> {
        ...
        if !base_dir.exists() {
            fs::create_dir_all(&base_dir).map_err(|e| {
                DomainError::IoError(format!("Failed to create database directory: {}", e))
            })?;
        }
        ...
        let conn = Connection::open(&db_path).map_err(|e| {
            DomainError::DatabaseConnection(format!("Failed to open SQLite database: {}", e))
        })?;
    ...
    fn init_database(&self) -> Result<()> {
        ...
        conn.pragma_update(None, "journal_mode", "WAL")
            .map_err(|e| {
                DomainError::DatabaseConnection(format!("Failed to enable WAL mode: {}", e))
            })?;
        conn.pragma_update(None, "synchronous", "NORMAL")
            .map_err(|e| {
                DomainError::DatabaseConnection(format!("Failed to set synchronous PRAGMA: {}", e))
            })?;
        conn.pragma_update(None, "foreign_keys", "ON")
            .map_err(|e| {
                DomainError::DatabaseConnection(format!(
                    "Failed to enable foreign keys PRAGMA: {}",
                    e
                ))
            })?;
    ...
    fn apply_migrations(&self) -> Result<()> {
        ...
        let tx = conn.transaction().map_err(|e| {
            DomainError::DatabaseConnection(format!("Failed to start migration transaction: {}", e))
        })?;
        ...
        .map_err(|e| {
            DomainError::DatabaseQuery(format!("Failed to create audit_logs table: {}", e))
        })?;

        tx.commit().map_err(|e| {
            DomainError::DatabaseConnection(format!("Failed to commit migrations: {}", e))
        })?;
...
impl ConnectionProvider for DatabaseManager {
    fn get_connection(&self) -> Result<MutexGuard<'_, Connection>> {
        self.connection
            .lock()
            .map_err(|_| DomainError::DatabaseConnection("Mutex poisoned".into()))
    }
}
