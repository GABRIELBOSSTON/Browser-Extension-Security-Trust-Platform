...
impl ScanRepository for SqliteScanRepository {
    ...
    fn get_scan(&self, id: &ScanId) -> Result<Option<ScanRecord>> {
        ...
        if let Some(row) = rows.next().map_err(|e| DomainError::DatabaseQuery(e.to_string()))? {
            let id_str: String = row.get(0).map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
            let extension_id: String = row.get(1).map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
            let extension_name: String = row.get(2).map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
            let version: String = row.get(3).map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
            let risk_score_f64: f64 = row.get(4).map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
            let severity_str: String = row.get(5).map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
            let timestamp_i64: i64 = row.get(6).map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
            let raw_json: String = row.get(7).map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;

            let uuid = Uuid::parse_str(&id_str).map_err(|e| DomainError::DatabaseSerialization(e.to_string()))?;
            let risk_score = RiskScore::new(risk_score_f64).map_err(|e| DomainError::DatabaseSerialization(e.to_string()))?;
            ...
            let scan_timestamp_utc = Utc.timestamp_opt(timestamp_i64, 0)
                .single()
    ...
    fn get_recent_scans(&self, limit: usize) -> Result<Vec<ScanRecord>> {
        ...
        let row_iter = stmt.query_map(rusqlite::params![limit as i64], |row| {
            ...
            Ok((id_str, extension_id, extension_name, version, risk_score_f64, severity_str, timestamp_i64, raw_json))
        }).map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;

        let mut results = Vec::new();
        for row_result in row_iter {
            let (id_str, extension_id, extension_name, version, risk_score_f64, severity_str, timestamp_i64, raw_json) = row_result.map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
            ...
            let scan_timestamp_utc = Utc.timestamp_opt(timestamp_i64, 0).single().unwrap_or_else(Utc::now);
    ...
    fn delete_scan(&self, id: &ScanId) -> Result<()> {
        let conn = self.provider.get_connection()?;
        conn.execute("DELETE FROM scans WHERE id = ?1", rusqlite::params![id.0.to_string()])
            .map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
        Ok(())
    }
    ...
    fn exists(&self, id: &ScanId) -> Result<bool> {
        let conn = self.provider.get_connection()?;
        let mut stmt = conn.prepare("SELECT 1 FROM scans WHERE id = ?1 LIMIT 1")
            .map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
        let mut rows = stmt.query(rusqlite::params![id.0.to_string()])
            .map_err(|e| DomainError::DatabaseQuery(e.to_string()))?;
        Ok(rows.next().map_err(|e| DomainError::DatabaseQuery(e.to_string()))?.is_some())
    }
...
mod tests {
        ...
        ScanRecord {
            ...
            report: ScanReport {
                risk_assessment: RiskAssessment {
                    ...
                    severity: Severity::Medium,
                    breakdown: vec![],
                    explanation: ScoreExplanation { top_contributors: vec![], breakdown_count: 0 },
                    diagnostics: vec![],
                },
                findings_count: 0,
            },
    ...
    fn test_save_and_get_scan() {
        ...
        let fetched = repo.get_scan(&record.id).expect("Failed to get scan").expect("Scan not found");
