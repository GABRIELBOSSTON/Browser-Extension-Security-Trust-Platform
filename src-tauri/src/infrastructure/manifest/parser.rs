...
impl ManifestParser {
    pub fn parse_file(path: &Path) -> Result<RawManifest> {
        if !path.exists() || !path.is_file() {
            return Err(DomainError::ManifestFileNotFound(
        ...
        if metadata.len() > MAX_MANIFEST_SIZE_BYTES {
            return Err(DomainError::ManifestFileTooLarge(
                path.to_string_lossy().to_string(),
            ));
        }
        ...
        file.read_to_string(&mut content).map_err(|e| {
            DomainError::IoError(format!("Failed to read string (invalid UTF-8?): {}", e))
        })?;
