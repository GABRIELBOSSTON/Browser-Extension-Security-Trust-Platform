...
impl ArchiveExtractorRegistry for ExtractorRegistryImpl {
    fn get_extractor(&self, source_archive: &Path) -> Result<Arc<dyn ArchiveExtractor>> {
        let mut file = File::open(source_archive).map_err(|e| {
            DomainError::IoError(format!("Failed to open file for registry: {}", e))
        })?;
...
mod tests {
    ...
    impl ArchiveExtractor for DummyZipExtractor {
        fn extract(
            &self,
            _src: &Path,
            _dest: &crate::domain::extraction::SandboxContext,
        ) -> Result<usize> {
            Ok(0)
        }
