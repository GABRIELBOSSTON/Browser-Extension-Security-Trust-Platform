...
impl ArchiveExtractor for ZipExtractor {
    fn extract(
        ...
        let mut archive = ZipArchive::new(file).map_err(|_| DomainError::CorruptedArchive)?;
        ...
        for i in 0..archive.len() {
            ...
            if !normalized_target.starts_with(&normalized_sandbox) {
                return Err(DomainError::ZipSlipDetected(format!(
            ...
            if (*file.name()).ends_with('/') {
                ...
                if let Some(p) = target_path.parent() {
                    if !p.exists() {
                        fs::create_dir_all(p).map_err(|e| DomainError::IoError(e.to_string()))?;
                    }
                }
                let mut outfile =
                    File::create(&target_path).map_err(|e| DomainError::IoError(e.to_string()))?;
