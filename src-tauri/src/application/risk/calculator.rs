...
impl RiskCalculatorService {
    ...
    pub fn calculate(&self, findings: &[StructuralFinding]) -> RiskAssessment {
        ...
        for finding in findings {
            grouped_findings
                .entry(finding.rule_id.0.to_string())
                .or_default()
                .push(finding);
        }
        ...
        for (rule_id, group) in grouped_findings {
            ...
            if let Some(config) = config_opt {
                ...
                let cumulative_weight = match &config.policy {
                    ...
                    AggregationPolicy::Decay => w * 2.0 * (1.0 - (0.5_f64).powi(n as i32)),
                };
                ...
                diagnostics.push(format!(
                    "WARN: Missing configuration for RuleId({})",
                    rule_id
                ));
            }
        }
        ...
        let clamped_val = rounded_raw.clamp(0.0, 100.0);
...
mod tests {
    use super::*;
    use crate::domain::rule_matcher::{DetectorKind, FindingId, RuleId};
    ...
    fn test_decay_aggregation_asymptote() {
        let configs = vec![RiskRuleConfig {
    ...
    fn test_max_and_once_aggregation() {
        let configs = vec![
            RiskRuleConfig {
                rule_id: "30".to_string(),
                base_weight: 45.0,
                policy: AggregationPolicy::Max,
            },
            RiskRuleConfig {
                rule_id: "40".to_string(),
                base_weight: 12.0,
                policy: AggregationPolicy::Once,
            },
        ];
        let service = RiskCalculatorService::new(configs, RiskProfile::Default);
        ...
        let mut findings = vec![];
        for _ in 0..5 {
            findings.push(make_finding(30));
        }
        for _ in 0..3 {
            findings.push(make_finding(40));
        }

        let assessment = service.calculate(&findings);
        assert_eq!(assessment.raw_score, 57.0); // 45.0 + 12.0

        let max_breakdown = assessment
            .breakdown
            .iter()
            .find(|b| b.rule_id == "30")
            .unwrap();
    ...
    fn test_missing_rule_id_diagnostics() {
        ...
        assert_eq!(
            assessment.diagnostics[0],
            "WARN: Missing configuration for RuleId(999)"
        );
    }
