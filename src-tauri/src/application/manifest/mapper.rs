...
use crate::domain::entities::{
    ActionConfig, BackgroundConfig, ContentScriptConfig, IconConfig, Manifest, 
    ManifestMetadata, ManifestVersion, Permission, PermissionSet, WebAccessibleResourceConfig
};
...
impl ManifestMapper {
    pub fn map(raw: RawManifest) -> crate::domain::errors::Result<Manifest> {
        ...
        let web_accessible_resources = Self::map_web_accessible_resources(raw.web_accessible_resources, version);
    ...
    fn map_background(raw: &Option<serde_json::Value>, version: ManifestVersion) -> Option<BackgroundConfig> {
        ...
        match version {
            ManifestVersion::V2 => {
                ...
                let page = obj.get("page").and_then(|v| v.as_str()).map(|s| s.to_string());
                let persistent = obj.get("persistent").and_then(|v| v.as_bool());
                Some(BackgroundConfig::V2 { scripts, page, persistent })
            },
            ManifestVersion::V3 => {
                let service_worker = obj.get("service_worker").and_then(|v| v.as_str())?.to_string();
                let worker_type = obj.get("type").and_then(|v| v.as_str()).map(|s| s.to_string());
                Some(BackgroundConfig::V3 { service_worker, worker_type })
            }
        }
    }

    fn map_action(action: &Option<serde_json::Value>, browser_action: &Option<serde_json::Value>, page_action: &Option<serde_json::Value>, version: ManifestVersion) -> Option<ActionConfig> {
        let obj = match version {
            ManifestVersion::V3 => action.as_ref()?.as_object()?,
            ManifestVersion::V2 => browser_action.as_ref().or(page_action.as_ref())?.as_object()?,
        };
        let default_popup = obj.get("default_popup").and_then(|v| v.as_str()).map(|s| s.to_string());
        let default_title = obj.get("default_title").and_then(|v| v.as_str()).map(|s| s.to_string());
        Some(ActionConfig { default_popup, default_title })
    }
    ...
    fn map_content_scripts(raw: Vec<serde_json::Value>) -> Vec<ContentScriptConfig> {
        ...
        for val in raw {
            if let Some(obj) = val.as_object() {
                ...
                let run_at = obj.get("run_at").and_then(|v| v.as_str()).map(|s| s.to_string());
                
                scripts.push(ContentScriptConfig { matches, js, css, run_at });
            }
        }
        scripts
    }

    fn map_web_accessible_resources(raw: Vec<serde_json::Value>, version: ManifestVersion) -> Vec<WebAccessibleResourceConfig> {
        let mut res = Vec::new();
        for val in raw {
            match version {
                ManifestVersion::V2 => {
